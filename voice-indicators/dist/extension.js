(function (swc, common, pluginData, events, dom, utils, modals, toasts, patcher) {
    ...
    const socket = lookup("https://ccwss.armagan.rest/voice-indicators", {
  ...
  `;
    ...
    async function fetchUserVoiceState(userId) {
    ...
    function patchUpdater() {
      patchContainer.add((() => {
        let interval = setInterval(() => {
          events__default["default"].emit("VoiceIndicators:EverySecond");
        }, 1e3);
        return () => {
  ...
  `;
    ...
    let syncCache = {};
    const tht = (user, elm) => {
      if (!(Date.now() - (syncCache[user.id] || 0) > 100))
        return;
